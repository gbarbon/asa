...
//      /r.accept()
  ...
  private[abstract_types] case class ArrayAt
    ...
    def merge(r: ArrayAt) = {
      ...
      big.copy(length = length merge r.length, creationImplicit = creationImplicit union r.creationImplicit, elements = felems)
    }
    ...
    def set(idx: ValueWithAbstraction, x: ValueWithAbstraction): ArrayAt = {
      idx match {
        case SingleValueWithAbstraction(index: AbstractNumWrapper, iuid) =>
          val i = index.content
          if (i.isPoint) {
            //The index is exact
            val idx = i.getLeft
    ...
    def join(r: ArrayAt): ArrayAt = {
      ...
      big.copy(length = length join r.length, creationImplicit = creationImplicit join r.creationImplicit, elements = felems)
    }
    def widening(r: ArrayAt): ArrayAt = {
      ...
      big.copy(length = length widening r.length, creationImplicit = creationImplicit widening r.creationImplicit, elements = felems)
    }
    def meet(r: ArrayAt): ArrayAt = {
      val (big, small) = if (elements.length <= r.elements.length) (this, r) else (r, this)
      val felems =
        big.elements.zip(small.elements).map{ case ((bv, bp), (sv, sp)) => (bv meet sv, bp || sp)}
      big.copy(length = length meet r.length, creationImplicit = creationImplicit meet r.creationImplicit, elements = felems)
    }
  }
  object ArrayAtFact {
    def create(ty: Type, length: Int, creationImplicit: InCADInfo, default: ValueWithAbstraction): ArrayAt = {
      default match {
        ...
        case darr: AbstractArray =>
          val elements = (for (_ <- Range (0, length) ) yield (darr joinADInfo creationImplicit, true) ).toVector
          ArrayAt (ty, SingleValueWithAbstraction(new AbstractNumWrapper(NumAt.fromNum(length)), creationImplicit), creationImplicit, elements)
  ...
  class AbstractArrayWrapper(cnt: ArrayAt) extends AbsArray {
    ...
    override def set(i: ValueWithAbstraction, x: ValueWithAbstraction): AbsArray = {
      new AbstractArrayWrapper(content.set(i, x))
    }
    ...
    override def getValues: Vector[AbstractValue] = {
      for ((v, _) <- content.elements) yield v
    }
  }
