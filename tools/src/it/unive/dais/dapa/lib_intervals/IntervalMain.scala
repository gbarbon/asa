...
object IntervalMain {
  ...
  def arith(b: itv_t, c: itv_t): Unit =
  {
    ...
    //itv_add(a,b,c);
    printf(s"$b + $c =")
    itv_print(itv_add(b, c))
    printf("\n")

    //itv_set(b,bb); itv_set(c,cc);
    //itv_sub(a,b,c);
    printf(s"$b - $c =")
    itv_print(itv_sub(b, c))
    printf("\n")

    //itv_set(b,bb); itv_set(c,cc);
    //itv_mul(intern,a,b,c);
    printf(s"$b * $c =")
    itv_print(itv_mul(b, c))
    printf("\n")

    //itv_set(b,bb); itv_set(c,cc);
    //itv_div(intern,a,b,c);
    printf(s"$b / $c =")
    itv_print(itv_div(b, c))
    printf("\n")

    //itv_set(b,bb); itv_set(c,cc);
    //itv_mod(intern,a,b,c);
    printf(s"$b mod $c =")
    itv_print(itv_mod(b, c))
    printf("\n")

    printf(s"$b JOIN $c =")
    itv_print(itv_join(b, c))
    printf("\n")

    printf(s"$b MEET $c =")
    itv_print(itv_meet(b, c)._2)
    printf("\n")

    printf(s"$b WIDENING $c =")
    itv_print(itv_widening(b, c))
    printf("\n")
