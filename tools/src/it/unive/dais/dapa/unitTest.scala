...
object unitTest {
  ...
  def booleanUnitTest(): Unit = {
    ...
    println("!= (inequality) between topBool and bottomBool is: " + topBool.!=^(bottomBool))
  ...
  def numUnitTest(): Unit = {
    ...
    println("theRes = {10,10} + {-42,-42}: " + theRes)
    ...
    println("theRes = {10, 10} + {-3,6}: " + theRes)
    theRes = theRes.-^(anotherNum)
    println("theRes = theRes - {-42,-42}: " + theRes)
    theRes = theRes.*^(anotherNum)
    println("theRes = theREs * {-42,-42}: " + theRes)
    ...
    println("{-oo,42} <= {24,+oo}: " +  anOpenLeftNum.<=^(anOpenRightNum))
    println("{-oo,42} >= {24,+oo}: " +  anOpenLeftNum.>=^(anOpenRightNum))
    println("negation applied to {24,+oo}: " + anOpenRightNum.negAt)
